...
void BitcoinGUI::createActions()
{
	...
	sendCoinsAction->setStatusTip(tr(exodusEnabled ? "Send Exodus and Blackholechain transactions" : "Send coins to a Blackholechain address"));
	...
	receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and Blackholechain: URIs)"));
    ...
    xnodeAction = new QAction(platformStyle->SingleColorIcon(":/icons/xnodes"), tr("&Xnodes"), this);
    xnodeAction->setStatusTip(tr("Browse xnodes"));
    xnodeAction->setToolTip(xnodeAction->statusTip());
    xnodeAction->setCheckable(true);
#ifdef Q_OS_MAC
    xnodeAction->setShortcut(QKeySequence(Qt::CTRL + key++));
#else
    xnodeAction->setShortcut(QKeySequence(Qt::ALT +  key++));
#endif
    tabGroup->addAction(xnodeAction);
#endif
...
#ifdef ENABLE_WALLET
    connect(xnodeAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(xnodeAction, SIGNAL(triggered()), this, SLOT(gotoXnodePage()));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Blackholechain addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Blackholechain addresses"));
    ...
    openAction->setStatusTip(tr("Open a Blackholechain: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Blackholechain command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    xnodeAction->setEnabled(enabled);
...
void BitcoinGUI::gotoXnodePage()
{
    QSettings settings;
    xnodeAction->setChecked(true);
    if (walletFrame) walletFrame->gotoXnodePage();
}
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Blackholechain network", "", count));
}
...
void BitcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    ...
    if(!xnodeSync.IsBlockchainSynced())
    {
...
void BitcoinGUI::setAdditionalDataSyncProgress(int count, double nSyncProgress)
{
    ...
    if(xnodeSync.IsBlockchainSynced())
    {
        ...
        strSyncStatus = QString(xnodeSync.GetSyncStatus().c_str());
...
{
    QString strTitle = tr("Blackholechain"); // default title
    ...
    // Append title to "Blackholechain - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
