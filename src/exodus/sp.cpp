...
CMPSPInfo::CMPSPInfo(const boost::filesystem::path& path, bool fWipe)
{
    ...
    implied_exodus.url = "https://www.ssc.moe";
    implied_exodus.data = "Exodus serve as the binding between Blackholechain, smart properties and contracts created on the Exodus Layer.";
    ...
    implied_texodus.url = "https://www.ssc.moe";
    implied_texodus.data = "Test Exodus serve as the binding between Blackholechain, smart properties and contracts created on the Exodus Layer.";
        ...
        std::pair<int64_t, int64_t>& tokens, bool& close_crowdsale)
{
    ...
    // Precision for all non-blackholechain values (bonus percentages, for example)
    ...
    // Precision for blackholechain amounts (satoshi)
