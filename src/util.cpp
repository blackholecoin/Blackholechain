...
const char * const BITCOIN_CONF_FILENAME = "Blackholechain.conf";
const char * const BITCOIN_PID_FILENAME = "Blackholechaind.pid";
...
#else
    const char* pszModule = "Blackholechain";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Blackholechain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Blackholechain
    // Mac: ~/Library/Application Support/Blackholechain
    // Unix: ~/.Blackholechain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Blackholechain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Blackholechain";
#else
    // Unix
    return pathRet / ".Blackholechain";
#endif
#endif
}
    ...
    if (!streamConfig.good())
     {
            ...
            if (configFile != NULL)
            {
                std::string strHeader =
                        ...
                        "#externalip=123.123.123.123:16900 ## Replace with your node external IP\n";
