...
void CDarksendPool::ProcessMessage(CNode *pfrom, std::string &strCommand, CDataStream &vRecv) {
    if (fLiteMode) return; // ignore all Blackholechain related functionality
    ...
    if (strCommand == NetMsgType::DSACCEPT) {
        ...
        if (!fXNode) {
            LogPrintf("DSACCEPT -- not a Xnode!\n");
        ...
        CXnode *pmn = mnodeman.Find(activeXnode.vin);
        if (pmn == NULL) {
        ...
        CXnode *pmn = mnodeman.Find(dsq.vin);
        if (pmn == NULL) return;

        if (!dsq.CheckSignature(pmn->pubKeyXnode)) {
        ...
        if (dsq.fReady) {
            if (!pSubmittedToXnode) return;
            if ((CNetAddr) pSubmittedToXnode->addr != (CNetAddr) pmn->addr) {
                LogPrintf("DSQUEUE -- message doesn't match current Xnode: pSubmittedToXnode=%s, addr=%s\n", pSubmittedToXnode->addr.ToString(), pmn->addr.ToString());
                return;
            }

            if (nState == POOL_STATE_QUEUE) {
                LogPrint("privatesend", "DSQUEUE -- PrivateSend queue (%s) is ready on xnode %s\n", dsq.ToString(), pmn->addr.ToString());
                SubmitDenominate();
            }
        } else {
            BOOST_FOREACH(CDarksendQueue
            q, vecDarksendQueue) {
                if (q.vin == dsq.vin) {
                    ...
                    LogPrint("privatesend", "DSQUEUE -- Xnode %s is sending WAY too many dsq messages\n", pmn->addr.ToString());
                    return;
                }
            }
            ...
            if (pmn->nLastDsq != 0 && nThreshold > mnodeman.nDsqCount) {
                LogPrint("privatesend", "DSQUEUE -- Xnode %s is sending too many dsq messages\n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            LogPrint("privatesend", "DSQUEUE -- new PrivateSend queue (%s) from xnode %s\n", dsq.ToString(), pmn->addr.ToString());
            if (pSubmittedToXnode && pSubmittedToXnode->vin.prevout == dsq.vin.prevout) {
                dsq.fTried = true;
            }
        ...
        //check it like a transaction
        {
...

            {
                ...
                if (!AcceptToMemoryPool(mempool, validationState, CTransaction(tx), true, false, NULL, false, true, true)) {
        ...
        if (fXNode) {
            // LogPrintf("DSSTATUSUPDATE -- Can't run on a Xnode!\n");
            return;
        }

        if (!pSubmittedToXnode) return;
        if ((CNetAddr) pSubmittedToXnode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSSTATUSUPDATE -- message doesn't match current Xnode: pSubmittedToXnode %s addr %s\n", pSubmittedToXnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (!fXNode) {
            LogPrintf("DSSIGNFINALTX -- not a Xnode!\n");
            return;
        }
        ...
        if (fXNode) {
            // LogPrintf("DSFINALTX -- Can't run on a Xnode!\n");
            return;
        }

        if (!pSubmittedToXnode) return;
        if ((CNetAddr) pSubmittedToXnode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSFINALTX -- message doesn't match current Xnode: pSubmittedToXnode %s addr %s\n", pSubmittedToXnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (fXNode) {
            // LogPrintf("DSCOMPLETE -- Can't run on a Xnode!\n");
            return;
        }

        if (!pSubmittedToXnode) return;
        if ((CNetAddr) pSubmittedToXnode->addr != (CNetAddr) pfrom->addr) {
            LogPrint("privatesend", "DSCOMPLETE -- message doesn't match current Xnode: pSubmittedToXnode=%s  addr=%s\n", pSubmittedToXnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
std::string CDarksendPool::GetStatus() {
    ...
    if ((pCurrentBlockIndex && pCurrentBlockIndex->nHeight - nCachedLastSuccessBlock < nMinBlockSpacing) || !xnodeSync.IsBlockchainSynced())
    ...
    switch (nState) {
        ...
        case POOL_STATE_QUEUE:
            ...
            return strprintf(_("Submitted to xnode, waiting in queue %s"), strSuffix);;
        case POOL_STATE_ACCEPTING_ENTRIES:
            if (nEntriesCount == 0) {
                ...
                if (nStatusMessageProgress % 70 <= 40) return strprintf(_("Submitted following entries to xnode: %u / %d"), nEntriesCount, GetMaxPoolTransactions());
                else if (nStatusMessageProgress % 70 <= 50) strSuffix = ".";
                else if (nStatusMessageProgress % 70 <= 60) strSuffix = "..";
                else if (nStatusMessageProgress % 70 <= 70) strSuffix = "...";
                return strprintf(_("Submitted to xnode, waiting for more entries ( %u / %d ) %s"), nEntriesCount, GetMaxPoolTransactions(), strSuffix);
            }
        case POOL_STATE_SIGNING:
...
//
// Check the mixing progress and send client updates if a Xnode
//
...
void CDarksendPool::CommitFinalTransaction() {
    if (!fXNode) return; // check and relay final tx only on xnode
...

    {
        ...
        if (!lockMain || !AcceptToMemoryPool(mempool, validationState, finalTransaction, true, false, NULL, false, true, true)) {
    ...
    // create and sign xnode dstx transaction
    if (!mapDarksendBroadcastTxes.count(hashTx)) {
        CDarksendBroadcastTx dstx(finalTransaction, activeXnode.vin, GetAdjustedTime());
        dstx.Sign();
...
//
// How does this work? Messages to Xnodes come in via NetMsgType::DSVIN, these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the Xnode
...
void CDarksendPool::ChargeFees() {
    ...
    if (nState == POOL_STATE_ACCEPTING_ENTRIES || nState == POOL_STATE_SIGNING) {
        ...
        bool fMissingInputs;
        if (!AcceptToMemoryPool(mempool, state, vecOffendersCollaterals[0], true, false, &fMissingInputs, false, true)) {
...
/*
    ...
    allow endless transaction that would bloat Blackholechain and make it unusable. To
...
void CDarksendPool::ChargeRandomFees() {
    ...
    const CTransaction &txCollateral, vecSessionCollaterals) {
        ...
        CValidationState state;
        bool fMissingInputs;
        if (!AcceptToMemoryPool(mempool, state, txCollateral, true, false, &fMissingInputs, false, true)) {
            // should never really happen
...
void CDarksendPool::CheckTimeout() {
    ...
    if (!fEnablePrivateSend && !fXNode) return;
    ...
    int nLagTime = fXNode ? 0 : 10000; // if we're the client, give the server a few extra seconds before resetting.
...
void CDarksendPool::CheckForCompleteQueue() {
    ...
    if (nState == POOL_STATE_QUEUE && IsSessionReady()) {
        ...
        CDarksendQueue dsq(nSessionDenom, activeXnode.vin, GetTime(), true);
...
bool CDarksendPool::IsCollateralValid(const CTransaction &txCollateral) {
...

    {
        LOCK(cs_main);
        CValidationState validationState;
        if (!AcceptToMemoryPool(mempool, validationState, txCollateral, true, false, NULL, false, true, true)) {
...
//
// Execute a mixing denomination via a Xnode.
// This is only ran from clients
//
bool CDarksendPool::SendDenominate(const std::vector <CTxIn> &vecTxIn, const std::vector <CTxOut> &vecTxOut) {
    if (fXNode) {
        LogPrintf("CDarksendPool::SendDenominate -- PrivateSend from a Xnode is not supported currently.\n");
        return false;
    }
    ...
    // we should already be connected to a Xnode
    if (!nSessionID) {
        LogPrintf("CDarksendPool::SendDenominate -- No Xnode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return false;
    }
...
// Incoming message from Xnode updating the progress of mixing
bool CDarksendPool::CheckPoolStateUpdate(PoolState nStateNew, int nEntriesCountNew, PoolStatusUpdate nStatusUpdate, PoolMessage nMessageID, int nSessionIDNew) {
    ...
    strAutoDenomResult = _("Xnode:") + " " + GetMessageByID(nMessageID);

    // if rejected at any state
    if (nStatusUpdate == STATUS_REJECTED) {
        LogPrintf("CDarksendPool::CheckPoolStateUpdate -- entry is rejected by Xnode\n");
        UnlockCoins();
        SetNull();
        SetState(POOL_STATE_ERROR);
...
bool CDarksendPool::SignFinalTransaction(const CTransaction &finalTransactionNew, CNode *pnode) {
    ...
    // push all of our signatures to the Xnode
    LogPrintf("CDarksendPool::SignFinalTransaction -- pushing sigs to the xnode, finalMutableTransaction=%s", finalMutableTransaction.ToString());
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun) {
    if (!fEnablePrivateSend || fXNode || !pCurrentBlockIndex) return false;
    ...
    if (mnodeman.size() == 0) {
        LogPrint("privatesend", "CDarksendPool::DoAutomaticDenominating -- No Xnodes detected\n");
        strAutoDenomResult = _("No Xnodes detected.");
        return false;
    }
    ...
    // If we've used 90% of the Xnode list then drop the oldest first ~30%
    int nThreshold_high = nMnCountEnabled * 0.9;
    int nThreshold_low = nThreshold_high * 0.7;
    LogPrint("privatesend", "Checking vecXnodesUsed: size: %d, threshold: %d\n", (int) vecXnodesUsed.size(), nThreshold_high);

    if ((int) vecXnodesUsed.size() > nThreshold_high) {
        vecXnodesUsed.erase(vecXnodesUsed.begin(), vecXnodesUsed.begin() + vecXnodesUsed.size() - nThreshold_low);
        LogPrint("privatesend", "  vecXnodesUsed: new size: %d, threshold: %d\n", (int) vecXnodesUsed.size(), nThreshold_high);
    }
    ...
    if (nLiquidityProvider || fUseQueue) {
        ...
        BOOST_FOREACH(CDarksendQueue & dsq, vecDarksendQueue)
        {
            ...
            CXnode *pmn = mnodeman.Find(dsq.vin);
            if (pmn == NULL) {
                LogPrintf("CDarksendPool::DoAutomaticDenominating -- dsq xnode is not in xnode list, xnode=%s\n", dsq.vin.prevout.ToStringShort());
                continue;
            }
            ...
            vecXnodesUsed.push_back(dsq.vin);
            ...
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt to connect to xnode from queue, addr=%s\n", pmn->addr.ToString());
            // connect to Xnode and submit the queue request
            CNode *pnode = (pnodeFound && pnodeFound->fXnode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
            if (pnode) {
                pSubmittedToXnode = pmn;
                ...
                strAutoDenomResult = _("Error connecting to Xnode.");
                continue;
            }
        }
    }
    ...
    while (nTries < 10) {
        CXnode *pmn = mnodeman.FindRandomNotInVec(vecXnodesUsed, MIN_PRIVATESEND_PEER_PROTO_VERSION);
        if (pmn == NULL) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Can't find random xnode!\n");
            strAutoDenomResult = _("Can't find random Xnode.");
            return false;
        }
        vecXnodesUsed.push_back(pmn->vin);

        if (pmn->nLastDsq != 0 && pmn->nLastDsq + nMnCountEnabled / 5 > mnodeman.nDsqCount) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Too early to mix on this xnode!"
                              " xnode=%s  addr=%s  nLastDsq=%d  CountEnabled/5=%d  nDsqCount=%d\n",
        ...
        LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt %d connection to Xnode %s\n", nTries, pmn->addr.ToString());
        CNode *pnode = (pnodeFound && pnodeFound->fXnode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
        if (pnode) {
    ...
    strAutoDenomResult = _("No compatible Xnode found.");
    return false;
}
...
bool CDarksendPool::CreateNewSession(int nDenom, CTransaction txCollateral, PoolMessage &nMessageIDRet) {
    if (!fXNode || nSessionID != 0) return false;
    ...
    if (!fUnitTest) {
        ...
        CDarksendQueue dsq(nDenom, activeXnode.vin, GetTime(), false);
...
bool CDarksendPool::AddUserToExistingSession(int nDenom, CTransaction txCollateral, PoolMessage &nMessageIDRet) {
    if (!fXNode || nSessionID == 0 || IsSessionReady()) return false;
...
std::string CDarksendPool::GetMessageByID(PoolMessage nMessageID) {
    switch (nMessageID) {
        ...
        case ERR_MN_LIST:
            return _("Not in the Xnode list.");
        case ERR_MODE:
            return _("Incompatible mode.");
        case ERR_NON_STANDARD_PUBKEY:
            return _("Non-standard public key detected.");
        case ERR_NOT_A_MN:
            return _("This is not a Xnode.");
        case ERR_QUEUE_FULL:
            return _("Xnode queue is full.");
        case ERR_RECENT:
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn &txin, const CPubKey &pubkey) {
    ...
    if (GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        ...
        if (out.nValue == XNODE_COIN_REQUIRED * COIN && out.scriptPubKey == payee) return true;
    }
...
bool CDarksendQueue::Sign() {
    ...
    return CheckSignature(activeXnode.pubKeyXnode);
}

bool CDarksendQueue::CheckSignature(const CPubKey &pubKeyXnode) {
    std::string strMessage = vin.ToString() + boost::lexical_cast<std::string>(nDenom) + boost::lexical_cast<std::string>(nTime) + boost::lexical_cast<std::string>(fReady);
    std::string strError = "";

    if (!darkSendSigner.VerifyMessage(pubKeyXnode, vchSig, strMessage, strError)) {
        LogPrintf("CDarksendQueue::CheckSignature -- Got bad Xnode queue signature: %s; error: %s\n", ToString(), strError);
        return false;
    }
...
bool CDarksendBroadcastTx::CheckSignature(const CPubKey &pubKeyXnode) {
    std::string strMessage = tx.GetHash().ToString() + boost::lexical_cast<std::string>(sigTime);
    std::string strError = "";
...
void CDarksendPool::RelayIn(const CDarkSendEntry &entry) {
    ...
    CNode *pnode = FindNode(pSubmittedToXnode->addr);
    if (pnode != NULL) {
...
void CDarksendPool::SetState(PoolState nStateNew) {
    if (fXNode && (nStateNew == POOL_STATE_ERROR || nStateNew == POOL_STATE_SUCCESS)) {
        LogPrint("privatesend", "CDarksendPool::SetState -- Can't set state to ERROR or SUCCESS as a Xnode. \n");
        return;
    }
...
void CDarksendPool::UpdatedBlockTip(const CBlockIndex *pindex) {
    ...
    if (!fLiteMode && xnodeSync.IsXnodeListSynced()) {
        NewBlock();
    }
}
...
void ThreadCheckDarkSendPool() {
    if (fLiteMode) return; // disable all Blackholechain specific functionality
    ...
    while (true) {
        ...
        xnodeSync.ProcessTick();

        if (xnodeSync.IsBlockchainSynced() && !ShutdownRequested()) {

            nTick++;

            // make sure to check all xnodes first
            mnodeman.Check();

            // check if we should activate or ping every few minutes,
            // slightly postpone first run to give net thread a chance to connect to some peers
            if (nTick % XNODE_MIN_MNP_SECONDS == 15)
                activeXnode.ManageState();

            if (nTick % 60 == 0) {
                mnodeman.ProcessXnodeConnections();
                mnodeman.CheckAndRemove();
                mnpayments.CheckAndRemove();
                instantsend.CheckAndRemove();
            }
            if (fXNode && (nTick % (60 * 5) == 0)) {
