...
UniValue exodus_getproperty(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"issuer\" : \"address\",            (string) the Blackholechain address of the issuer on record\n"
...
UniValue exodus_getorderbook(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "[                                              (array of JSON objects)\n"
            "  {\n"
            "    \"address\" : \"address\",                         (string) the Blackholechain address of the trader\n"
...
UniValue exodus_gettradehistoryforaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            ...
            "  {\n"
            "    \"txid\" : \"hash\",                               (string) the hex-encoded hash of the transaction of the order\n"
            "    \"sendingaddress\" : \"address\",                  (string) the Blackholechain address of the trader\n"
            ...
            "        \"address\" : \"address\",                         (string) the Blackholechain address of the other trader\n"
...
UniValue exodus_gettradehistoryforpair(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "    \"address\" : \"address\",                 (string) the Blackholechain address of the seller\n"
            "    \"amountsold\" : \"n.nnnnnnnn\",           (string) the number of tokens sold in this trade\n"
            "    \"amountreceived\" : \"n.nnnnnnnn\",       (string) the number of tokens traded in exchange\n"
            "    \"matchingtxid\" : \"hash\",               (string) the hash of the transaction that was matched against\n"
            "    \"matchingaddress\" : \"address\"          (string) the Blackholechain address of the other party of this trade\n"
            "  },\n"
            "  ...\n"
            "]\n"
            "\nExamples:\n"
...
UniValue exodus_getactivedexsells(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"seller\" : \"address\",               (string) the Blackholechain address of the seller\n"
            "    \"amountavailable\" : \"n.nnnnnnnn\",   (string) the number of tokens still listed for sale and currently available\n"
            "    \"blackholechaindesired\" : \"n.nnnnnnnn\",    (string) the number of blackholechains desired in exchange\n"
            "    \"unitprice\" : \"n.nnnnnnnn\" ,        (string) the unit price (BHC/token)\n"
            ...
            "      {\n"
            "        \"buyer\" : \"address\",                (string) the Blackholechain address of the buyer\n"
            ...
            "        \"amounttopay\" : \"n.nnnnnnnn\"        (string) the amount in blackholechains needed finalize the trade\n"
            "      },\n"
            "      ...\n"
            "    ]\n"
            "  },\n"
    ...
    for (OfferMap::iterator it = my_offers.begin(); it != my_offers.end(); ++it) {
        ...
        int64_t sellBitcoinDesired = selloffer.getBHCDesiredOriginal(); //badly named - "Original" implies off the wire, but is amended amount
        ...
        // calculate unit price and updated amount of blackholechain desired
        ...
        responseObj.push_back(Pair("blackholechaindesired", FormatDivisibleMP(bitcoinDesired)));
        ...
        for (AcceptMap::const_iterator ait = my_accepts.begin(); ait != my_accepts.end(); ++ait) {
            ...
            if (accept.getHash() == selloffer.getHash()) {
                ...
                int64_t amountToPayInBTC = calculateDesiredBTC(accept.getOfferAmountOriginal(), accept.getBHCDesiredOriginal(), amountAccepted);
...
UniValue exodus_getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"blackholechaincoreversion\" : \"x.x.x\",        (string) Blackholechain Core version\n"
    ...
    // provide the Exodus and Blackholechain version
    infoResponse.push_back(Pair("exodusversion_int", EXODUS_VERSION));
    infoResponse.push_back(Pair("exodusversion", ExodusVersion()));
    infoResponse.push_back(Pair("blackholechaincoreversion", BlackholechainCoreVersion()));
...
UniValue exodus_getsto(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "  \"recipients\": [                 (array of JSON objects) a list of recipients\n"
            "    {\n"
            "      \"address\" : \"address\",          (string) the Blackholechain address of the recipient\n"
