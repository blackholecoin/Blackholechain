...
bool IsBlockPayeeValid(const CTransaction &txNew, int nBlockHeight, CAmount blockReward) {
    ...
    if (!sporkManager.IsSporkActive(SPORK_4_XNODE_PAYMENT_START)) {
        ...
        if (fDebug) LogPrintf("IsBlockPayeeValid1 -- xnode isn't start\n");
        return true;
    }
...
bool CXnodeBlockPayees::IsTransactionValid(const CTransaction &txNew) {
    ...
    LogPrintf("CXnodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f BHC\n", strPayeesPossible, (float) nXnodePayment / COIN);
    return false;
}
