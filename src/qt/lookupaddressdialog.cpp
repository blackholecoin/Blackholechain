...
void LookupAddressDialog::searchAddress()
{
    ...
    // lets see if the string is a valid Blackholechain address
    ...
    if (address.IsValid()) //do what?
    {
        ...
        ui->balanceLabel->setText(QString::fromStdString(FormatDivisibleMP(getUserAvailableMPbalance(searchText, 1)) + " EXODUS"));
        // QR
        #ifdef USE_QRCODE
        ui->QRCode->setText("");
        ...
        for (pItem = 1; pItem < 11; pItem++)
        {
            if (pFound[pItem])
            {
                ...
                if (pName[pItem]=="Test Exodus (#2)") { tokenLabel = " TEXODUS"; }
                if (pDivisible[pItem])
                {
