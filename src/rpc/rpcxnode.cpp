...
UniValue privatesend(const UniValue &params, bool fHelp) {
    ...
    if (params[0].get_str() == "start") {
        ...
        if (fXNode)
            return "Mixing is not supported from xnodes";
...
//    obj.push_back(Pair("mixing_mode",       fPrivateSendMultiSession ? "multi-session" : "normal"));
    ...
    if (darkSendPool.pSubmittedToXnode) {
        obj.push_back(Pair("outpoint", darkSendPool.pSubmittedToXnode->vin.prevout.ToStringShort()));
        obj.push_back(Pair("addr", darkSendPool.pSubmittedToXnode->addr.ToString()));
    }
...
UniValue xnode(const UniValue &params, bool fHelp) {
    ...
    if (fHelp ||
        ...
        throw std::runtime_error(
                "xnode \"command\"...\n"
                        ...
                        "1. \"command\"        (string or set of strings, required) The command to execute\n"
                        "\nAvailable commands:\n"
                        "  count        - Print number of all known xnodes (optional: 'ps', 'enabled', 'all', 'qualify')\n"
                        "  current      - Print info on current xnode winner to be paid the next block (calculated locally)\n"
                        ...
                        "  winner       - Print info on next xnode winner to vote for\n"
    ...
    if (strCommand == "connect") {
        if (params.size() < 2)
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Xnode address required");
        ...
        if (!pnode)
            throw JSONRPCError(RPC_INTERNAL_ERROR, strprintf("Couldn't connect to xnode %s", strAddress));
    ...
    if (strCommand == "count") {
        ...
        int nCount;
        mnodeman.GetNextXnodeInQueueForPayment(true, nCount);
    ...
    if (strCommand == "current" || strCommand == "winner") {
        ...
        winner = mnodeman.GetNextXnodeInQueueForPayment(nHeight, true, nCount);
        ...
        obj.push_back(Pair("lastseen", (winner->lastPing == CXnodePing()) ? winner->sigTime :
                                       winner->lastPing.sigTime));
        obj.push_back(Pair("activeseconds", (winner->lastPing == CXnodePing()) ? 0 :
    ...
    if (strCommand == "debug") {
        if (activeXnode.nState != ACTIVE_XNODE_INITIAL || !xnodeSync.IsBlockchainSynced())
            return activeXnode.GetStatus();

        CTxIn vin;
        CPubKey pubkey;
        CKey key;

        if (!pwalletMain || !pwalletMain->GetXnodeVinAndKeys(vin, pubkey, key))
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                               "Missing xnode input, please look at the documentation for instructions on xnode creation");
    ...
    if (strCommand == "start") {
        if (!fXNode)
            throw JSONRPCError(RPC_INTERNAL_ERROR, "You must set xnode=1 in the configuration");
        ...
        if (activeXnode.nState != ACTIVE_XNODE_STARTED) {
            activeXnode.nState = ACTIVE_XNODE_INITIAL; // TODO: consider better way
    ...
    if (strCommand == "start-alias") {
        ...
        BOOST_FOREACH(CXnodeConfig::CXnodeEntry mne, xnodeConfig.getEntries()) {
            if (mne.getAlias() == strAlias) {
                ...
                bool fResult = CXnodeBroadcast::Create(mne.getIp(), mne.getPrivKey(), mne.getTxHash(),
...
//        LogPrintf("start-alias: statusObj=%s\n", statusObj);
    ...
    if (strCommand == "start-all" || strCommand == "start-missing" || strCommand == "start-disabled") {
        ...
        if ((strCommand == "start-missing" || strCommand == "start-disabled") &&
            !xnodeSync.IsXnodeListSynced()) {
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
                               "You can't use this command until xnode list is synced");
        }
        ...
        returnObj.push_back(Pair("overall",
                                 strprintf("Successfully started %d xnodes, failed to start %d, total %d",
    ...
    if (strCommand == "status") {
        if (!fXNode)
            throw JSONRPCError(RPC_INTERNAL_ERROR, "This is not a xnode");

        UniValue mnObj(UniValue::VOBJ);

        mnObj.push_back(Pair("vin", activeXnode.vin.ToString()));
        mnObj.push_back(Pair("service", activeXnode.service.ToString()));

        CXnode mn;
        if (mnodeman.Get(activeXnode.vin, mn)) {
            mnObj.push_back(Pair("payee", CBitcoinAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
        }

        mnObj.push_back(Pair("status", activeXnode.GetStatus()));
        return mnObj;
    }
    ...
    if (strCommand == "winners") {
        ...
        if (params.size() > 3)
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Correct usage is 'xnode winners ( \"count\" \"filter\" )'");
...
UniValue xnodelist(const UniValue &params, bool fHelp) {
    ...
    if (fHelp || (
        ...
        throw std::runtime_error(
                "xnodelist ( \"mode\" \"filter\" )\n"
                        ...
                        "                   (since latest issued \"xnode start/start-many/start-alias\")\n"
                        ...
                        "  payee          - Print Blackholechain address associated with a xnode (can be additionally filtered,\n"
                        "                   partial match)\n"
                        ...
                        "  qualify        - Print qualify status of a xnode based on current block\n"
                        "  status         - Print xnode status: PRE_ENABLED / ENABLED / EXPIRED / WATCHDOG_EXPIRED / NEW_START_REQUIRED /\n"
...
bool DecodeHexVecMnb(std::vector <CXnodeBroadcast> &vecMnb, std::string strHexMnb) {
...
UniValue xnodebroadcast(const UniValue &params, bool fHelp) {
    std::string strCommand;
    if (params.size() >= 1)
        strCommand = params[0].get_str();
    ...
    if (fHelp ||
        ...
        throw std::runtime_error(
                "xnodebroadcast \"command\"...\n"
                        "Set of commands to create and relay xnode broadcast messages\n"
                        "\nArguments:\n"
                        "1. \"command\"        (string or set of strings, required) The command to execute\n"
                        "\nAvailable commands:\n"
                        "  create-alias  - Create single remote xnode broadcast message by assigned alias configured in xnode.conf\n"
                        "  create-all    - Create remote xnode broadcast messages for all xnodes configured in xnode.conf\n"
                        "  decode        - Decode xnode broadcast message\n"
                        "  relay         - Relay xnode broadcast message to the network\n"
        );
    ...
    if (strCommand == "create-alias") {
        ...
        UniValue statusObj(UniValue::VOBJ);
        std::vector <CXnodeBroadcast> vecMnb;

        statusObj.push_back(Pair("alias", strAlias));

        BOOST_FOREACH(CXnodeConfig::CXnodeEntry
        mne, xnodeConfig.getEntries()) {
            if (mne.getAlias() == strAlias) {
                fFound = true;
                std::string strError;
                CXnodeBroadcast mnb;
    ...
    if (strCommand == "create-all") {
        ...
        std::vector <CXnodeConfig::CXnodeEntry> mnEntries;
        mnEntries = xnodeConfig.getEntries();
        ...
        returnObj.push_back(Pair("overall", strprintf(
                "Successfully created broadcast messages for %d xnodes, failed to create %d, total %d",
                nSuccessful, nFailed, nSuccessful + nFailed)));
        returnObj.push_back(Pair("detail", resultsObj));
    ...
    if (strCommand == "decode") {
        if (params.size() != 2)
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Correct usage is 'xnodebroadcast decode \"hexstring\"'");

        std::vector <CXnodeBroadcast> vecMnb;

        if (!DecodeHexVecMnb(vecMnb, params[1].get_str()))
            throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Xnode broadcast message decode failed");
        ...
        BOOST_FOREACH(CXnodeBroadcast & mnb, vecMnb)
        {
            UniValue resultObj(UniValue::VOBJ);
            ...
            if (mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyXnode", CBitcoinAddress(mnb.pubKeyXnode.GetID()).ToString()));
                ...
                nFailed++;
                resultObj.push_back(Pair("errorMessage", "Xnode broadcast signature verification failed"));
            }
        ...
        returnObj.push_back(Pair("overall", strprintf(
                "Successfully decoded broadcast messages for %d xnodes, failed to decode %d, total %d",
                nSuccessful, nFailed, nSuccessful + nFailed)));
    ...
    if (strCommand == "relay") {
        if (params.size() < 2 || params.size() > 3)
            throw JSONRPCError(RPC_INVALID_PARAMETER, "xnodebroadcast relay \"hexstring\" ( fast )\n"
                    "\nArguments:\n"
        ...
        BOOST_FOREACH(CXnodeBroadcast & mnb, vecMnb)
        {
            ...
            if (mnb.CheckSignature(nDos)) {
                if (fSafe) {
                    fResult = mnodeman.CheckMnbAndUpdateXnodeList(NULL, mnb, nDos);
                } else {
                    mnodeman.UpdateXnodeList(mnb);
                    mnb.RelayXNode();
                    fResult = true;
        ...
        returnObj.push_back(Pair("overall", strprintf(
                "Successfully relayed broadcast messages for %d xnodes, failed to relay %d, total %d", nSuccessful,
...
UniValue getmasternodecount (const UniValue& params, bool fHelp) {
    ...
    if (chainActive.Tip())
        mnodeman.GetNextXnodeInQueueForPayment(chainActive.Tip()->nHeight, true, nCount);
