...
if (nHeight > HF_F_PAYMENT_START)
    {
        CScript FOUNDER_1_SCRIPT;
		// Take some reward away from us
        coinbaseTx.vout[0].nValue = -0.5 * COIN;
        FOUNDER_1_SCRIPT = GetScriptForDestination(CBitcoinAddress("B5ZQV1tQRaq3hzLvuN263UEbRsz7fFmWqu").Get());
        coinbaseTx.vout.push_back(CTxOut(0.5 * COIN, CScript(FOUNDER_1_SCRIPT.begin(), FOUNDER_1_SCRIPT.end())));
    }
...
//            }
            ...
            if (tx.IsSigmaMint() || tx.IsSigmaSpend()) {
                ...
                if(sigmaState->IsSurgeConditionDetected()) {
                    LogPrintf("Miner: SurgeConditionDetected\n");
                    continue;
                }
                if (sporkManager.IsSporkActive(SPORK_9_SIGMA_NEW)) {
                    LogPrintf("Miner: SPORK_9_SIGMA_NEW\n");
                    continue;
                }
            }
        ...
        if ((nHeight >= HF_F_PAYMENT_START) && (nHeight <=770000))
        {
            ...
            FillBlockPayments(coinbaseTx, nHeight, xnodePayment, pblock->txoutXnode);
        }
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock, true, NULL, false))
        return error("BlackholechainMiner: ProcessNewBlock, block not accepted");

    return true;
}

void static BlackholechainMiner(const CChainParams &chainparams) {
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("blackholechain-miner");
    ...
    try {
        ...
        if (!coinbaseScript || coinbaseScript->reserveScript.empty()) {
            LogPrintf("BlackholechainMiner stop here coinbaseScript=%s, coinbaseScript->reserveScript.empty()=%s\n", coinbaseScript, coinbaseScript->reserveScript.empty());
        ...
        while (true) {
            if (chainparams.MiningRequiresPeers()) {
                ...
                // Also try to wait for xnode winners unless we're on regtest chain
                do {
                    bool fvNodesEmpty;
                    {
                        LOCK(cs_vNodes);
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("Error in BlackholechainMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running BlackholechainMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        LogPrintf("BlackholechainMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e) {
        LogPrintf("BlackholechainMiner runtime error: %s\n", e.what());
        return;
    }
}
...
void GenerateBitcoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BlackholechainMiner, boost::cref(chainparams)));
}
